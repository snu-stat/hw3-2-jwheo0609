...
점추정치 비교: logit_model의 추정치($\beta_1$)는 약 1.75(정확한 추정치는 1.75320)으로, 1항의 최소제곱법이나 로그 변환 선형 회귀로 구한 $k$ 값(정확한 추정치는 1.75279)과 거의 일치합니다.

통계적 의미: 1항에서 사용한 로그 변환 선형 회귀(lm 사용)는 승률을 단순한 연속형 변수로 취급하고 변환하여 회귀를 돌린 것입니다. 반면, 2항의 로지스틱 회귀(glm 사용)는 승과 패의 데이터의 이항 분포 구조를 반영한 보다 통계적으로 의미있는 모형입니다.

신뢰구간 비교: 값 자체는 1항과 매우 비슷하지만, glm 모형이 데이터의 이항 분포의 확률 분포 구조를 반영했기 때문에 통계학적으로 신뢰구간의 타당성은 로지스틱 모델이 좀더 높다고 평가할 수 있습니다.
...
print(plot_ii)
# plot_ii.show()
# plot_ii.save(filename="plot_ii.png", width=8, height=6, dpi=300)
...
print(plot_iii)
# plot_iii.show()
# plot_iii.save(filename="plot_iii.png", width=8, height=6, dpi=300)
print("python에서 image.show()가 되지 않아 quarto 이미지 불러오기로 대체합니다.")
```
...
판단 기준: 잔차들이 $Y=0$ (빨간 점선)을 기준으로 위아래로 무작위로 흩어져 있어야 좋은 모형입니다. 만약 U자형이나 역U자형 등의 곡선 패턴이 보이거나, 한쪽으로 갈수록 잔차가 커지는 나팔 모양이 나타난다면 모형에 구조적인 문제(비선형성 등)가 있음을 의미합니다. 피타고리안 승률 모형은 대개 잔차가 특정한 패턴 없이 0 주변에 고르게 분포하여 훌륭한 적합도를 보여줍니다.

iii. 관측된 WPct와 모형에서 예측하는 WPct 산점도해석  
방법: 모형이 예측한 승률을 X축으로, 실제 기록한 승률을 Y축으로 그립니다.  
...
분리형 로지스틱 모형을 적합한 결과, 실점(log_RA)의 계수는 음수($-1.75$)로 나타나고 득점(log_R)의 계수는 양수($+1.75$)로 나타납니다. 두 계수의 절대값은 매우 비슷하며, 이는 1항의 피타고리안 지수 $k$ 및 2항의 통합 계수 $\beta_1$의 값인 $1.75$와 거의 일치합니다. 이 결과가 가지는 통계학적 및 야구 기하학적 의미는 다음과 같습니다.

대칭성 증명: 득점 1%가 증가할 때 승리 오즈(Odds)가 증가하는 효율과, 실점 1%가 증가할 때 승리 오즈가 감소하는 효율의 크기가 거의 동일함을 데이터가 실증적으로 보여줍니다.  
모형의 간결성: 4항처럼 변수를 두 개로 분리하여 유의미하게 모델의 설명력이 높아지지 않으므로, 하나의 지수 $k$ 또는 하나의 계수 $\beta_1$만 사용하는 1항과 2항의 모형이 데이터를 매우 잘 요약하는 훌륭한 모형임을 반증합니다.
...
23개의 설명변수를 투입하여 절편이 있는 로지스틱 회귀 모형을 구축하고 AIC 기준 단계별 변수선택(Stepwise Selection)을 수행한 결과, 최종적으로 logRS, logRA, CG, SHO, SV 변수가 선택되었다. 그러나 이 모형을 그대로 채택하기에는 통계적, 도메인 지식적 측면에서 다음과 같은 심각한 오류가 존재한다.  
과적합과 통계적 비유의성: AIC 알고리즘에 의해 선택된 완투(CG)와 완봉(SHO)은 회귀계수 유의성 검정 결과 각각 p-value가 0.129, 0.065로 유의수준 5%에서 기각되었다. 이는 AIC 지표가 예측력을 미세하게 높이기 위해 불필요한 노이즈 변수까지 포함시키는 한계를 보여준다.  
내생성 변수로 인한 피타고리안 구조의 왜곡: 세이브(SV) 변수는 높은 유의성(p < 0.001)을 보였으나, 이는 팀이 승리하는 상황에서만 기록되는 사후적 지표로 인과관계의 역전을 발생시킨다. 실제로 SV가 모형에 포함됨으로써 득점(logRS, $\beta \approx 1.61$)과 실점(logRA, $\beta \approx -1.28$)의 회귀계수 크기가 축소되고, 문제 1과 2-1에서 확인했던 피타고리안 승률 모형의 완벽한 대칭성이 심하게 훼손되었다.  

[최종 결론 및 문제 1과의 비교]  
결과적으로, 통계적으로 유의하지 않은 희귀 스탯(CG, SHO)과 내생성 교란 변수(SV)를 수동으로 제거해야만 타당한 인과 추론이 가능하다.이를 모두 제거하고 나면 결국 logRS와 logRA만 남게 되며, 이는 빌 제임스의 피타고리안 승률 모형(문제 1)이 수많은 세부 스탯을 투입한 복잡한 모형보다 오히려 데이터의 본질적 구조를 더 정확하게 꿰뚫어보는 간결하고 우수한 모형임을 통계적으로 증명하는 결과이다.
...
승리 횟수(W)를 반응변수로 포아송 회귀 모형을 적합하고 단계적 변수선택을 수행한 결과, logRS, ERA, CG, E, SV가 선택되었다. 이 결과를 앞선 문제 2-2의 로지스틱 회귀 모형과 비교하면 다음과 같은 구조적 한계점들이 발견된다.

첫째, 수학적 대칭성 및 해석력의 저하이다. 로지스틱 모형은 logRS와 logRA를 대칭적으로 선택하여 빌 제임스의 피타고리안 승률 모형과 본질적으로 궤를 같이했다. 반면, 포아송 모형은 logRA 대신 비율 스탯인 ERA를 선택함으로써 득실점의 균형 있는 비교를 어렵게 만들었다.

둘째, 통계적 유의성 및 내생성 문제의 지속이다. 문제 2-2와 마찬가지로, AIC 기준은 통계적으로 유의하지 않은 노이즈 변수(CG, E)를 남겨 과적합 오류를 범했으며, 이기는 경기에서만 발생하는 세이브(SV) 변수가 여전히 극단적으로 높은 유의성을 보이며 인과 추론을 교란시켰다.

셋째, 포아송 분포 가정의 위배이다. 모형의 잔차 이탈도(Residual deviance, 62.29)가 자유도(444)에 비해 비정상적으로 작게 나타났다. 이는 162경기라는 상한선이 존재하고 설명변수들의 예측력이 너무 높은 야구 데이터 특성상, 평균과 분산이 같아야 한다는 포아송 모형의 기본 가정을 심각하게 위배하는 과소산포 현상이다.

최종 결론: 결과적으로 제한된 경기 수 내에서의 승리 비율을 다루는 야구 통계에서는, 카운트 데이터 기반의 포아송 모형보다 상한선이 통제된 이항 분포 기반의 로지스틱 회귀 모형(문제 2-2)이 통계적 가정과 야구의 도메인 지식 측면 모두에서 훨씬 타당하고 우수한 모형이다.
...
첫째, 모형의 산포 매개변수(Theta)가 약 1,085만이라는 비정상적으로 거대한 수치로 발산하였고, alternation limit reached 경고가 발생했습니다. 이는 투입된 설명변수들의 예측력이 너무 높아 잔차 분산이 평균보다 작아지는 과소산포 현상 때문이며, 과대산포를 해결하기 위해 고안된 음이항 모형이 이 데이터에 구조적으로 부적합함을 증명합니다.

둘째, logRA, ERA, H 등 팀 승리에 직결되는 핵심 변수들이 모두 통계적으로 유의하지 않게(p>0.05) 나타났습니다. 이는 변수 간의 극심한 다중공선성으로 인해 표준오차가 팽창했기 때문입니다. 반면, 이기는 경기에서만 발생하는 내생성 변수인 세이브(SV)는 비정상적으로 높은 유의성을 보였습니다.
